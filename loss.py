...
def normalize(x, axis=-1):
    ...
    """
    x = 1. * x / (nn.Norm(x, axis, keep_dims=True).expand_as(x) + 1e-12)
    return x
...
def euclidean_dist(x, y):
    ...
    xx = P.pow(x, 2).sum(1, keep_dims=True).expand(m, n)
    yy = P.pow(y, 2).sum(1, keep_dims=True).expand(n, m).T()
    dist = xx + yy
...
class CrossEntropyLabelSmooth(nn.Cell):
    ...
    def construct(self, inputs, targets):
        ...
        targets = ops.Zeros(log_probs.size())   #.scatter_(1, targets.unsqueeze(1).data.cpu(), 1)
